...
public final class SupplierResource extends RepresentationModel<SupplierResource> {
     ...
     */
    public SupplierResource(final CommonModel common, final SupplierModel model, final boolean withCommon) {
        this.common = withCommon ? common : null;
        this.model = model;

        if (withCommon) {
            add(linkTo(SupplierController.class).withRel("all"));
        }

        try {
            add(linkTo(methodOn(SupplierController.class).retrieveById(model.getId(), true)).withSelfRel());
    ...
    public boolean equals(final Object o) {
        ...
        final SupplierResource that = (SupplierResource) o;
