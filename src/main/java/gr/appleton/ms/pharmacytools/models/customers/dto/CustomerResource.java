...
public final class CustomerResource extends RepresentationModel<CustomerResource> {
     ...
     */
    public CustomerResource(final CommonModel common, final CustomerModel model, final boolean withCommon) {
        this.common = withCommon ? common : null;
        this.model = model;

        if (withCommon) {
            add(linkTo(CustomerController.class).withRel("all"));
        }

        try {
            add(linkTo(methodOn(CustomerController.class).retrieveById(model.getId(), true)).withSelfRel());
    ...
    public boolean equals(final Object o) {
        ...
        final CustomerResource that = (CustomerResource) o;
